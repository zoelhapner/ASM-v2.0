...
@if ($navbarItemHelper->isSubmenu($item))
    <div class="dropend {{ collect($item['submenu'])->contains(fn($sub) => request()->is(ltrim(parse_url($sub['url'] ?? '', PHP_URL_PATH), '/'))) ? 'show' : '' }}">
        <a class="dropdown-item dropdown-toggle {{ collect($item['submenu'])->contains(fn($sub) => request()->is(ltrim(parse_url($sub['url'] ?? '', PHP_URL_PATH), '/'))) ? 'active' : '' }}"
           href=""
           data-bs-toggle="dropdown" data-bs-auto-close="outside" role="button"
